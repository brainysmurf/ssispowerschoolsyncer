import datetime
import postgresql
from psmdlsyncer.utils.Dates import date_to_database_timestamp, today, tomorrow, yesterday
...
class HWDRDFU(RelativeDateFieldUpdater):
    ...
    def output(self):
        return "Lunch time on {day_of_week} ({date})"
...
class Homework_Club(DragonNetDBConnection):
    """
    ...
    def __init__(self, table):
        ...
        for row in potential_rows:
            ...
            record = self.sql("select * from {table} where recordid = {record_id} order by fieldid;".format(**d))()
    ...
    def define(self):
        ...
        # agents
        self.agents    = ['susancover@ssis-suzhou.net']
    ...
    def derive_content(self, item):
        ...
        date = item['date']
        return self.list(student + " given by " + teacher)
    ...
    def format_for_email(self):
        ...
        * Each item in the tag is a listed item in that tag
        ...
        self.html("<p>The following students have a Homework Detention:</p>")

        for item in self.final:
            self.html(self.derive_content(item))   # puts in the content

        if not self.final:
            self.html( "<p>No one has any today</p>" )
...
if __name__ == "__main__":
    ...
    try:
        here = Homework_Club('ssismdl_data_content')
